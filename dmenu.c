...
setup(void)
{
	...
	if (parentwin == root && (info = XineramaQueryScreens(dpy, &n))) {
		...
		if (centered) {
			mw = MIN(MAX(dmw > 0 ? dmw : max_textw() + promptw, min_width), info[i].width);
...
#endif
	{
		...
		if (centered) {
			mw = MIN(MAX(dmw > 0 ? dmw : max_textw() + promptw, min_width), wa.width);
			x = (wa.width  - mw) / 2;
			y = (wa.height - mh) / 2;
		} else {
			x = dmx;
...
usage(void)
{
	fputs("usage: dmenu [-bfinvx] [-l lines] [-p prompt] [-fn font] [-m monitor]\n"
	      ...
	      "             [-nb color] [-nf color] [-sb color] [-sf color] [-w windowid] [-s number]\n", stderr);
	exit(1);
}
...
main(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++)
		...
		if (!strcmp(argv[i], "-v")) {      /* prints version information */
			...
			fstrstr = cistrstr;
		} else if (!strcmp(argv[i], "-u"))   /* invert use_prefix */
		...
		/* these options take one argument */
			...
			embed = argv[++i];
		else if (!strcmp(argv[i], "-s"))   /* preselected item */
